...
class CreateProvidersActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_create_providers)
        ...
        guardarButton.setOnClickListener{
            ...
            val file = File(path)
            MeetAndTravelApi.requestProviderRegister(prefs!!.userToken!!, prefs!!.userId.toString(), file, buildEvent(), { response -> handleEventResponse(response) }, { error -> handleEventError(error)})
        }
        ...
        Log.d("Debug","Proveedor Creado")
    }
    private fun setupActionBar() {
        supportActionBar?.title = "Registrar Proveedor"
    }

    private fun buildEvent(): JSONObject {
        val jsonObject = JSONObject()
        jsonObject.put("name", nameEditText.text)
        jsonObject.put("ruc", rucEditText.text)
        jsonObject.put("description", descriptionEditText.text)
        jsonObject.put("location", locationEditText.text)
        jsonObject.put("telephone",  telephoneEditText.text)
        jsonObject.put("web_address", web_addressEditText.text)
        Log.d(MeetAndTravelApi.tag, jsonObject.toString())
        return jsonObject
    }
    private fun handleEventResponse(response: NetworkResponse?) {
        Toast.makeText(this, "Proveedor creado satisfactoriamente", Toast.LENGTH_SHORT).show()
